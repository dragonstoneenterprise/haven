...
- Tier 3 Tasks 1 & 2 done (nav cleanup 8→5 chips; orders pagination client-side 25/page) — **not committed**. Task 0 (card reconciliation) and Task 3 (welcome bonus fields) blocked on the Amex 2004 question below.
- Multi-bank-account plan written, not started.

### Cards
| Card | Rate | Limit | LLC | Orders | Notes |
|---|---|---|---|---|---|
| Amex 5008 (app still shows "Chase Amazon" — rename) | 5% | $700 | Cryptic Dragon | 102 | main BFMR card; replaced 4001 (same account, also 5%) |
| Chase Amazon Visa 5899 | 3% | $3,000 | Cryptic Dragon | 2 | rate corrected from 5% |
| Citi Double Cash 4397 | 2% | $1,840 | Cryptic Dragon | 0 | |
| Coinbase 4173 | 2% | $4,000 | Dragonstone | 0 | |
| US Bank 9413 | 2% | ~$500 (verify) | — | 0 | Amazon gift card purchases; 3 statement rows, **no card row yet** |
| Amex 2004 | 2% | ? | — | 0 | **see blocker**; 64 statement rows, no card row yet |
| Amazon prime (orphan) | 5% | $700 | none | 0 | keep orphaned + archived (see Decisions) |

Chase Sapphire 3856 is personal — no BFMR purchases, not tracked. Statement rows persisted in Supabase: Amex 5008 (125), 2004 (64), Chase 5899 (95), Citi 4397 (25), 9413 (3).

Key insight: gift-card orders still earn 5% because the gift cards were purchased on Amex 5008.

### Blocker: Amex 2004
Assumed 2004 is 5008's renewed number, but the data contradicts it: 2004 and 5008 charged concurrently every month for 18 months, 2004 busier in early 2025, and each has independent payments (2004: 18 payments totaling $3,938.74) — looks like two separate accounts. Earlier records agree: 5008 replaced **4001** (same account, both 5%), not 2004, and 2004 was rated 2% — a different cashback tier. One order matches only a 2004 charge; deleting those 64 rows drops reconcile 75/77 → 74/77.
...
- Money as integer cents (BIGINT); dollars only at render. Orders store **per-unit** money; UI multiplies by `qty` everywhere.
- `qty` authoritative from BFMR — always synced on import.
...
- **Never sum bank balances across LLCs** (would show $561.59 as one company's cash).
...
- **Cashback computed at import time** and **stored per-order** (`expected_cashback_pct/cents`), not looked up from card — imported orders get the same profit math as manual orders, and card changes don't alter historical math.
...
- **Write-back to BFMR intentionally deferred** — `POST /api/v2/my-tracker` (order numbers/tracking) exists in the API; Floatless stays read-only for now.
...
- **Multi-bank plan uses expand/contract**: new `bank_accounts` table (FK to `bfmr_account_id`) → backfill → cutover → verify → drop old columns.
...
### Reference — BFMR integration & pages
...
- **Tracker Import** pulls `GET /api/v2/my-tracker` (all pages), upserts by `bfmr_reserve_id`, syncing qty/status/tracking/amazon_order_id. Field mapping: `item_name`→title, `qty` (string→int), `retailer_links[0].retailer`, `item_model_number`→notes.
...
- Points-card support: `is_points_card` + `point_value_cents`; earn rate × point value = effective % for deal math.
...
### Reference — schema
`profiles` (user profile; `bank_name` legacy) · `orders` (`cost_cents`, `payout_cents`, `cashback_pct`, `expected_cashback_cents`, `expected_profit_cents`, `status`, `bfmr_reserve_id`, `bfmr_purchase_id`, `imported_from_bfmr`, `bfmr_account_id`, `qty`) · `cards` (`default_cashback_pct`, `is_points_card`, `point_value_cents`, `bfmr_account_id`) · `scanned_deals` (`deal_id`, `margin_bps`, `is_special_deal`, `dismissed`, `bfmr_account_id`) · `bfmr_accounts` (`name`, `api_key`, `api_secret`, `bank_balance_cents`, `bank_name`, `is_default`) · `card_statements` (all card CSV transactions, persisted) · `amazon_orders` (persisted; unique on `user_id + order_id + model_number`).

Migrations 001–010 through the multi-account era; pre-010 JSON backups at `~/Desktop/floatless/backups/`.

### Reference — reconciliation, 5 matching methods in priority order
...
### Shipped history (kept for context)
